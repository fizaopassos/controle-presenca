...
// Normaliza "operador" => "lancador" (compatível com ENUM do banco)
if (perfil === 'operador') perfil = 'lancador';
...
// gestor e lançador vinculam condomínios (admin não precisa)
...
const usuarioEditado = rowsUsuario[0];
...
// Busca permissões (condomínios que este usuário pode enxergar)
...
// Importante: passamos usuarioEditado, não sobrescrevemos o usuario da sessão
res.render('layout', {
...
perfil = perfil.toLowerCase();
// Normaliza "operador" => "lancador"
if (perfil === 'operador') perfil = 'lancador';

// Impedir e-mail duplicado em outro usuário
...
// gestor / lançador vinculam condomínios (admin não precisa)
if (perfil !== 'admin' && condominios) {
  const condoArray = Array.isArray(condominios) ? condominios : [condominios];
