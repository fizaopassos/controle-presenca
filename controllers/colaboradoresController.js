...
let { nome, empresa_id, condominio_id, posto_id, tipo, data_inicio } = req.body;
...
if (tipoFinal === 'fixo') {
  if (!condominio_id) {
    return res.status(400).send('Para colaboradores FIXOS, o condomínio é obrigatório.');
  }
  if (!posto_id) {
    return res.status(400).send('Para colaboradores FIXOS, o posto é obrigatório.');
  }
}

// Permissão: checa condomínio final (somente quando for FIXO)
if (tipoFinal === 'fixo' && condominio_id && user.perfil !== 'admin') {
...
// data_inicio: somente para FIXO
let dataInicioFinal = null;
if (tipoFinal === 'fixo') {
  if (!data_inicio || String(data_inicio).trim() === '') {
    dataInicioFinal = hojeISODate();
  } else {
    const v = String(data_inicio).trim();
    if (!isISODate(v)) {
      return res.status(400).send('Data de início inválida (use YYYY-MM-DD).');
    }
    dataInicioFinal = v;
  }
}
...
const sqlInsert =
  'INSERT INTO colaboradores (nome, empresa_id, condominio_id, posto_id, tipo, ativo, data_inicio) ' +
  'VALUES (?, ?, ?, ?, ?, TRUE, ?)';
...
const { id } = req.params;
let { nome, empresa_id, condominio_id, posto_id, tipo, ativo, data_inicio } = req.body;
...
// Busca atual (para: garantir que existe + permitir manter data_inicio se não vier no form)
const [rowsAtual] = await db.query(
  'SELECT id, tipo, data_inicio FROM colaboradores WHERE id = ? LIMIT 1',
  [id]
);
...
// Permissão: checar condomínio final (somente se vai ficar FIXO)
const condFinal = tipoFinal === 'cobertura' ? null : condominio_id;
if (tipoFinal === 'fixo' && condFinal && user.perfil !== 'admin') {
  const [rowsPerm] = await db.query(
    `SELECT 1
     FROM usuario_condominios
     WHERE usuario_id = ? AND condominio_id = ?
  ...
  if (rowsPerm.length === 0) {
    return res.status(403).send('Você não tem permissão para alterar colaborador neste condomínio.');
  }
}

// Verifica se já existe outro colaborador com mesmo nome na mesma empresa
const [[existe]] = await db.query(
  `SELECT id FROM colaboradores
   WHERE nome = ?
     AND empresa_id = ?
...
if (tipoFinal === 'fixo') {
  if (data_inicio && String(data_inicio).trim() !== '') {
    ...
    // mantém o atual; se ainda não tiver, usa hoje
    if (atual.data_inicio) {
      const s = typeof atual.data_inicio === 'string'
        ? atual.data_inicio.slice(0, 10)
        : hojeISODate();
      dataInicioFinal = s;
    } else {
      dataInicioFinal = hojeISODate();
